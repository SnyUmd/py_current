...
# **********************************************************************************
# 画像内のテキストを抽出　に必要なソフトウェア
# Windows
# https://github.com/UB-Mannheim/tesseract/wiki

# 上記より、tesseract-ocr-w32-setup-v5.1.0.20220510.exe　をダウンロード
# tesseract-ocr-w32-setup-v5.1.0.20220510.exe
...
aryMsg = ["[win] + [ctrl] + %s  =  クリップボードの画像からテキストを抽出" % aryKeys[0],
           ...
           "[win] + [ctrl] + %s  =  日本語→英語翻訳ツール(ウインドウ)" % aryKeys[5],
...
# マルチスレッド
# タスクトレイ表示もループになっている為、タスクトレイアイコンを表示させる為には、
# ループにする必要があるもよう。
t_icon.start() #タスクトレイにアイコンを表示するスレッド
t_main.start() #メインスレッド
               # ショートカットキー監視
               # 実行処理
